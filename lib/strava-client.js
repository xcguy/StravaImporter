...
const _rateLimit = async (ratelimits) => {
  ...
  if (ratelimits.shortTermUsage / ratelimits.shortTermLimit > 0.9){
    ...
    console.log(`getting close to short term limit. Waiting for ${cooldown / ONE_SEC} seconds, ${ new Date(new Date().getTime() + cooldown).toLocaleTimeString()} `)
  ...
  if (ratelimits.longTermUsage / ratelimits.longTermLimit > 0.95){
    ...
    console.log(`getting close to long term limit. Waiting till midnight (UTC) to continue (${cooldown / ONE_MIN} mins), ${ new Date(new Date().getTime() + cooldown).toLocaleTimeString()} `)
    await sleep(cooldown + FIVE_SEC)
  }
  return false;
}
